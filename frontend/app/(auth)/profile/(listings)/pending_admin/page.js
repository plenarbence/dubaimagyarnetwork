...
export default function PendingAdminListingsPage() {
  ...
  const fetchListings = useCallback(async () => {
    try {
      ...
      const filtered = data.filter((l) => l.status === "pending_admin");
  ...
  return (
    <div className="max-w-2xl mx-auto p-4">
      <h1 className="text-2xl font-semibold mb-6 text-center">
        Jóváhagyásra váró hirdetéseid
      </h1>

      {listings.length === 0 ? (
        <p className="text-center text-gray-600">
          Nincs admin jóváhagyásra váró hirdetésed. ✅
        </p>
      ) : (
